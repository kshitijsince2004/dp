...
> **SUPERSEDED (2026-09-25).** The July 2026 normalisation removed `records.data`. The live model is a `records` spine plus typed detail tables (`fir_details`, `arrest_details`, …), `persons` (+ subtypes), `record_properties`, `record_offences`, and `locations`, routed by `field_registry.storage` via `records.mapper.js`. See [`docs/PHAROS_REVERSE_ENGINEERING_BLUEPRINT.md`](docs/PHAROS_REVERSE_ENGINEERING_BLUEPRINT.md) and [`docs/DB_SCHEMA.md`](docs/DB_SCHEMA.md). The historical decision text below is retained for provenance only — do not implement against it.
...
> **SUPERSEDED in part (2026-09-25).** Ownership is split: Node.js `report-engine` + `phq-diary` generate statutory diaries (`PHQ_DIARY`, `DISTRICT_DIARY`, `FN_DIARY`); the Python worker generates daily-diary / `dd-*` sheets only. See [`docs/REPORT_ENGINE_OWNERSHIP.md`](docs/REPORT_ENGINE_OWNERSHIP.md). Inline legacy templates that read `records.data` (`arrest-summary`, `pcr-call-log`, `cases-register`) are retired. The historical “Python for all Excel including PHQ” framing below is retained for provenance only.
...
- `backend/migrations/20260917000001_add_cause_of_death_other.js`
