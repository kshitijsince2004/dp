...
import {
  ...
  GROUP_LABELS,
  filterFieldsForRole,
} from './reportable-fields.config.js';
import {
  validateQuerySpec,
...
export const getMetadata = async (req, res) => {
  try {
    ...
    const { ROW_GRAIN_OPTIONS, SYSTEM_PRESET_SPECS } = await import('./reportable-fields.config.js');
...
export const getLookupValues = async (req, res) => {
  try {
    ...
    switch (type) {
      ...
      case 'crime-heads': {
        const { REPORTABLE_FIELDS: RF } = await import('./reportable-fields.config.js');
