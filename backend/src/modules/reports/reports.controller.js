...
/** Templates that read the removed records.data JSONB column — retired (H1). */
const RETIRED_LEGACY_TEMPLATE_IDS = new Set(['arrest-summary', 'pcr-call-log', 'cases-register']);

const assertTemplateNotRetired = (templateId) => {
  if (templateId && RETIRED_LEGACY_TEMPLATE_IDS.has(String(templateId))) {
    const err = new Error(
      `Template "${templateId}" is retired (read dead records.data). Use statutory diaries (PHQ/DISTRICT/FN via report-engine) or daily-diary/dd-* (Python worker).`
    );
    err.statusCode = 410;
    err.code = 'TEMPLATE_RETIRED';
    throw err;
  }
};

// Fallback in-memory templates (legacy arrest-summary / pcr-call-log / cases-register removed)
const templates = [
...
export const getTemplates = async (req, res) => {
  ...
  try {
    ...
    formatted = formatted.filter(t =>
      !RETIRED_LEGACY_TEMPLATE_IDS.has(String(t.id)) &&
      !RETIRED_LEGACY_TEMPLATE_IDS.has(String(t.code || '').toLowerCase())
    );
...
const getRecordsForReport = async (templateId, filters) => {
  ...
  assertTemplateNotRetired(templateId);
  let recordType = null;
  ...
  for (const [key, val] of Object.entries(filters)) {
    ...
    if (coreColumns.includes(key)) {
      ...
      log.warn('getRecordsForReport: skipping dynamic filter for non-core column', { templateId, key });
    }
  }
...
export const generateReport = async (req, res) => {
  ...
  try {
    assertTemplateNotRetired(template_id);
  } catch (err) {
    log.warn('generateReport: rejected — retired legacy template', { userId, template_id });
    return res.status(err.statusCode || 410).json({
      status: 'error',
      success: false,
      code: err.code || 'TEMPLATE_RETIRED',
      message: err.message
    });
  }
  ...
  try {
    ...
    if (!effectiveUserId) {
      log.warn('generateReport: rejected — no authenticated user id for created_by', { template_id });
      return res.status(401).json({
        status: 'error',
        success: false,
        code: 'UNAUTHORIZED',
        message: 'Authenticated user required to generate reports'
      });
    }
