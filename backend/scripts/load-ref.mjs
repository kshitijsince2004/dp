// load-ref — org hierarchy + the 21 ref.* lookup tables (DB_SCHEMA.md §8).
// Successor of the dead scripts/menu_table().js.
//
...
//   - beats: sheet ps_cd is an official numeric code; until the official PS-code mapping
//     file is supplied (config/org/ps_codes.json), beats load with ps_id = NULL and the
//     raw code kept in source_ps_cd — printed loudly as a deferred link
//   - overlay naming an unknown local_head_cd → ABORT
//
// Usage: npm run load-ref
...
const fail = (msg) => { console.error(`\n✗ load-ref FAILED: ${msg}`); process.exit(1); };
...
console.log(`Loading hierarchy (${hierarchy.length} nodes) …`);
const codeToId = {};
await db.transaction(async (trx) => {
  ...
  if (stale.length) {
    ...
    console.log(`hierarchy_nodes: deactivated ${stale.length} nodes absent from config: ${stale.map(s => s.code).slice(0, 10).join(', ')}${stale.length > 10 ? ' …' : ''}`);
  }
});
console.log(`hierarchy_nodes: ${hierarchy.length} upserted`);
...
if (junkSections.length) {
  console.log(`ref.sections: EXCLUDED ${junkSections.length} prose-spillover rows (NULL act_sec_cd) — row numbers: ${junkSections.map(j => j.rowNum).join(', ')}`);
}
...
});
{
  ...
  if (q.length) {
    console.log(`ref.major_minor_mapping: QUARANTINED ${mmmAll.length - mmm.length}/${mmmAll.length} source-dirty rows:`);
    for (const [reason, vals] of q) {
      console.log(`  unresolvable ${reason} (${vals.length} rows): distinct = ${[...new Set(vals)].slice(0, 20).join(', ')}`);
    }
  }
}
...
// beats — ps_id resolution deferred until the official PS-code mapping exists
...
if (psCodeMap) {
  ...
  console.log(`ref.beats: ${linked}/${beats.length} linked to PS via org/ps_codes.json`);
  if (notInMap.size) {
    console.log(`ref.beats: ⚠ ${notInMap.size} ps_cd values are NOT in the official code list (${beats.length - linked} beats stay ps_id=NULL, raw code kept): ${[...notInMap].sort().join(', ')}`);
  }
} else {
  console.log(`ref.beats: ⚠ DEFERRED LINKING — config/org/ps_codes.json not present (official PS-code list pending); loading all ${beats.length} beats with ps_id = NULL, raw code kept in source_ps_cd`);
}
