// sync-config — the ONE config-as-data sync command (ARCHITECTURE.md §7, DB_SCHEMA.md §6).
//
...
// duplicate keys. Usage: npm run sync-config
...
const fail = (msg) => { console.error(`\n✗ sync-config FAILED: ${msg}`); process.exit(1); };
...
async function syncTable(label, table, keyCol, items, toRow) {
  const existing = await db(table).select('*');
  const byKey = new Map(existing.map(r => [r[keyCol], r]));
  let inserted = 0, updated = 0, unchanged = 0, deactivated = 0;
  const seen = new Set();
  ...
  console.log(`${label.padEnd(28)} +${inserted} inserted, ~${updated} updated, =${unchanged} unchanged, -${deactivated} deactivated`);
}
...
try {
  ...
  await syncTable('fields → field_registry', 'field_registry', 'field_key', fields, (f) => ({
  ...
  await syncTable('workflow → transitions_config', 'workflow_transitions_config', 'code', workflow, (w) => ({
  ...
  await syncTable('proformas → report_templates', 'report_templates', 'code', proformas, (p) => ({
    name: p.name,
  ...
  await syncTable('contracts → level_data_contracts', 'level_data_contracts', 'code', contracts, (c) => ({
