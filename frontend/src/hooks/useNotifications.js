...
/**
 * Resolve the access token for notifications.
 * Live API: SuperTokens owns the session token.
 * Mock Mode: DebugBar stores a mock-jwt in localStorage.
 */
const resolveAccessToken = async () => {
  if (!isLiveApiMode()) return getAccessToken();
  try {
 ...
 * `?token=` (see sseAuthMiddleware). Only real JWTs in Live API mode may
 * open the stream — mock-jwt-* tokens from DebugBar Mock Mode must not.
 */
export function useNotifications() {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    log.debug('hook:notifications:fetch_start', { hasToken: true, mockToken: isMockAccessToken(token) });
  ...
  const connectSSE = useCallback(async () => {
    ...
    const token = await resolveAccessToken();
    if (!canUseRealtimeNotifications(token)) {
      log.debug('hook:notifications:sse_skip', {
        reason: !token ? 'no_token' : isMockAccessToken(token) ? 'mock_token' : 'not_live_api_mode',
      });
      closeEventSource();
      return;
    }
    ...
    es.onerror = () => {
      ...
      (async () => {
        ...
        if (!canUseRealtimeNotifications(latest)) {
