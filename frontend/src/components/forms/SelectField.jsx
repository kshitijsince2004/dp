...
export default function SelectField({
  ...
  if (!inputClassName) {
    if (variant === 'compact') {
      inputClassName = `w-full min-h-[36px] px-3.5 py-2 border-2 rounded-xl bg-white text-sm font-medium outline-none focus:border-blue-500 cursor-text disabled:bg-slate-50 disabled:text-slate-400 disabled:cursor-not-allowed ${
        isError ? 'border-red-400 bg-red-50 focus:border-red-500' : 'border-[#7a9cc5]'
      }`;
    } else {
      inputClassName = `w-full min-h-[36px] bg-white border-2 text-slate-800 text-sm px-4 py-2.5 rounded-xl outline-none focus:border-[var(--accent-color)] transition-colors disabled:bg-slate-50 disabled:text-slate-400 disabled:cursor-not-allowed cursor-text ${
