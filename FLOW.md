...
- **Fixed Crash on RecordDetail**: Added missing `Lock` icon import from `lucide-react` in `frontend/src/pages/sho/RecordDetail.jsx` which was causing an "Illegal constructor" error when rendering the Chargesheet badge.

- **ARREST Status Updates**: Added "Formal Arrest" to the Custody Status options for ARREST records. This was added to both against-FIR and Kalandra lists in `statusOptions.config.js`, to the Kalandra patch in `import-fields.config.js`, and to the ARREST flat union list in `template-builder.service.js`. No database migration was required as the column is unrestricted text.
